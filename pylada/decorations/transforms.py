...
class Transforms(object):
    ...
    def _enhance_lattice(self):
        ...
        for site in self.lattice:
            intermediate = set()
            for j in site.equivto:
                intermediate |= self.lattice[j].equivto

        for site in self.lattice:
            site.equivto = min(site.equivto)

        for i, site in enumerate(self.lattice):
            if not hasattr(site.type, '__iter__'):
                site.nbflavors = 1
            elif len(site.type) < 2:
                site.nbflavors = 1
            else:
    ...
    def invariant_ops(self, cell):
        """ boolean indices into invariant operations. 
            ...
            :type cell:
                :py:attr:`~pylada.crystal.Structure` or 3x3 matrix
    ...
    def toarray(self, hft, structure):
        ...
        for atom in structure:
            site = atom.site
            if self.lattice[site].nbflavors == 1:
                continue
