...
def copy_folder(self, event):
    ...
    for source, destination in pairs:
        ...
        if jobdest.is_executable and not args.force:
            ...
            a = ''
            while a not in ['n', 'y']:
                a = cmdl_input('{0} already exists. Overwrite? [y/n]'.format(jobdest.name))
            if a == 'n':
...
def delete_folder(self, event):
    ...
    if args.recursive:
        if not args.force:
            a = ''
            while a not in ['n', 'y']:
                a = cmdl_input("Delete {0} and its subfolders? [y/n]".format(cjf[folder].name))
            if a == 'n':
    ...
    # only delete specified jobfolder.
        ...
        if not args.force:
            a = ''
            while a not in ['n', 'y']:
                a = cmdl_input("Delete {0}? [y/n]".format(cjf[folder].name))
            if a == 'n':
                print(cjf[folder].name, "not deleted.")
                return
        jobfolder = cjf[folder]
...
def delete_completer(self, event):
    ...
    if len(noop) > 1:
        return result
    elif len(noop) == 1 and len(event.symbol) == 0:
        return result
