...
def layer(structure, direction, tolerance=1e-12):
    """ Iterates over layers and atoms in a layer.
        ...
        :param direction:
            ...
            Defaults to the first column vector of the structure. It is important that two
            cell-vectors of the structure are (or can be transformed to be) perpendicular to the
            growth direction. Otherwise it cannot be assured that layers are well defined, i.e. that
            each atom belongs to a single (as in periodic) layer. This condition is *not* enforced
            (no assertion) since it is not necessary, only sufficient. Note that the third vector
            need not be parallel to the growth direction.

        :param tolerance:
            Maximum difference between two atoms in the same layer.

        :returns:
            Yields iterators over atoms in a single epitaxial layer.
    """
...
def equivalence(structure, operations=None, tolerance=1e-6, splitocc=None):
    ...
    if operations == None:
        operations = space_group(structure)
