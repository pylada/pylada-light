...
def U(type=1, l=2, U=0e0, J=0e0):
    ...
    if hasattr(type, "lower"):
        type = type.lower()
        if type == "liechtenstein":
            type = 1
        elif type == "dudarev":
            type = 2
        else:
            raise ValueError("Unknown value for type: {0}.".format(type))
    if hasattr(l, "lower"):
        l = l.lower()
        if len(l) != 1:
            raise ValueError("Uknown input {0}.".format(l))
        if l[0] == 's':
            l = 0
        elif l[0] == 'p':
            l = 1
    ...
    if type != 1 and type != 2:
        raise ValueError("Unknown LDA+U type: {0}.".format(type))
...
class Specie(object):
    ...
    def __init__(self, directory, U=None, oxidation=None, **kwargs):
        ...
        if U is None:
            ...
            self.U = [u for u in U]  # takes care of any kind of iterator.
