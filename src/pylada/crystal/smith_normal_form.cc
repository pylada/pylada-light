...
namespace pylada {
namespace {
template <class T> bool one_nonzero(Eigen::MatrixBase<T> const &_in) {
...
template <class T> bool check_nonzero(Eigen::MatrixBase<T> const &_in, size_t _index) {
  int n(0);
...
void get_min_max(Eigen::MatrixBase<T> const &_in, types::t_int &_max, types::t_int &_min) {
  ...
  for(int k(1); k < _in.size(); ++k)
    if(std::abs(_in(k)) > std::abs(_in(_max)))
      _max = k;
  ...
  _min = k;
  for(--k; k >= 0; --k)
    if(_in(k) != 0 and std::abs(_in(k)) < std::abs(_in(_min)))
      _min = k;
}

template <class T>
void check_same_magnitude(Eigen::MatrixBase<T> const &_in, types::t_int &_max, types::t_int &_min,
                          size_t _index) {
  if(std::abs(_in(_min, _index)) != std::abs(_in(_max, _index)))
    return;
  int n0(0), n1(0);
  for(int i(0); i < _in.rows(); ++i) {
    if(_in(_max, i))
  ...
  if(n0 < n1 or (n0 == n1 and _in(_max, _index) < _in(_max, _index)))
    std::swap(_min, _max);
}

//! Column 0 to have only one non-zero positive component placed at origin.
template <class T1, class T2>
void smith_col_impl(Eigen::MatrixBase<T1> &_left, Eigen::MatrixBase<T2> &_smith, size_t _index) {
...
void smith_row_impl(Eigen::MatrixBase<T2> &_smith, Eigen::MatrixBase<T3> &_right, size_t _index) {
...
void smith_impl_(Eigen::MatrixBase<T1> &_left, Eigen::MatrixBase<T2> &_right,
    ...
    do {
      ...
      for(int i(index + 1); i < _smith.rows(); ++i)
        for(int j(index + 1); j < _smith.cols(); ++j) {
          if(_smith(i, j) % diag == 0)
            continue;
          else if(maxmod == 0) {
            maxrow = i;
            maxmod = std::abs(_smith(i, j) % diag);
...
void smith_normal_form(types::t_int *_S, types::t_int *_L, types::t_int *_R) {
  ...
  Eigen::Map<types::iMatrix3d> S(_S, 3, 3);
  Eigen::Map<types::iMatrix3d> L(_L, 3, 3);
  Eigen::Map<types::iMatrix3d> R(_R, 3, 3);
