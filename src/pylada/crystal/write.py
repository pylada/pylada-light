...
def poscar(structure, file='POSCAR', vasp5=None, substitute=None, direct=True):
    ...
    if file == None:
        ...
        with open(RelativePath(file).path, 'w') as file:
            file.write(string)
    else:
        file.write(string)
...
def crystal(structure, file='fort.34',
    ...
    try:
        ...
        if dimensionality < 0 or dimensionality > 3:
            dimensionality = 3
    except:
        dimensionality = 3
    try:
        if centering is None:
            centering = getattr(structure, 'centering', 1)
        centering = int(centering)
    except:
        centering = 1
    try:
        if type is None:
            type = getattr(structure, 'type', 1)
        type = int(type)
    except:
        type = 1
    if spacegroup is None:
        spacegroup = getattr(structure, 'spacegroup', None)
    if spacegroup is None:
        spacegroup = space_group(structure)
    if len(spacegroup) == 0:
...
def gulp(structure, file='gulp.in', **kwargs):
    ...
    if symmgroup is None:
        symmgroup = getvalue('spacegroup', None)
    ...
    if periodicity is None:
        periodicity = 3
        if abs(crystal.cell[2, 2] - 500) < 1e-8:
            periodicity -= 1
        if abs(crystal.cell[1, 1] - 500) < 1e-8:
            periodicity -= 1
        if abs(crystal.cell[0, 0] - 500) < 1e-8:
            periodicity -= 1
    if periodicity == 3:
        result += 'vectors\n'                                                      \
        ...
        raise error.NotImplementedError('Cannot do 1d materials')
    elif periodicity == 0:
        raise error.NotImplementedError('Cannot do 0d materials')
    ...
    if asymatoms is None:
        ...
        if symmops is None:
            symmops = getvalue('spacegroup', None)
    ...
    for key, value in regions.items():
        ...
        if key != 0:
            index = 'region {0}'.format(key)
        elif len(keyword) == 0:
            index = ''
        else:
            index = 'region {0}'.format(max(len(regions) + 1, 3))
