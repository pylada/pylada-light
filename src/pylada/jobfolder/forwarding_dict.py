...
class ForwardingDict(MutableMapping):
    ...
    def __init__(self, ordered=True, readonly=None, naked_end=None,
        ...
        self.__dict__[
            'only_existing'] = jobparams_only_existing if only_existing is None else only_existing
    ...
    def __setattr__(self, name, value):
        ...
        try:
            super(ForwardingDict, self).__getattribute__(name)
        ...
        if self.readonly:
            raise RuntimeError("ForwardingDict instance is read-only.")
        ...
        for item in self.dictionary.values():
            if attrs:  # unroll attribute list.
                try:
                    item = reduce(getattr, chain([item], self._attr_list))
                except AttributeError:
                    continue
    ...
    def __delattr__(self, name):
        ...
        try:
            super(ForwardingDict, self).__delattr__(name)
        except AttributeError:
            pass
        else:
            return
    ...
    def __setitem__(self, key, value):
        ...
        if self.readonly:
            raise RuntimeError("This ForwardingDict is readonly.")
        ...
        if len(self._attr_list) > 1:
            try:
                o = reduce(getattr, chain([o], self._attr_list[:-1]))
            except AttributeError:
