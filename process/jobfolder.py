...
class JobFolderProcess(Process):
    ...
    def _next(self):
        ...
        if len(self.process) >= self.nbpools:
            return
        ...
        try:
            ...
            while len(self.process) < self.nbpools \
                ...
                if name not in self.jobfolder:
                    raise IndexError(
                        "Job-folder {0} no longuer exists.".format(name))
                jobfolder = self.jobfolder[name]
                if not jobfolder.is_executable:
                    raise IndexError(
                        "Job-folder {0} is no longuer executable.".format(name))
                ...
                if hasattr(jobfolder.functional, 'iter'):
                    ...
                    process = CallProcess(self.functional, join(
                        self.outdir, name), **params)
    ...
    def start(self, comm):
        ...
        if super(JobFolderProcess, self).start(comm):
            return True
        self._next()
        return False
        ...
        for name, value in jobfolder.root.items():
            if value.is_tagged or name in running:
                ...
                for key, value in value.__dict__.items():
                    if key in ['children', 'params', '_functional', 'parent']:
                        continue
