...
class CallProcess(Process):
    ...
    def poll(self):
        ...
        if super(CallProcess, self).poll():
            return True
        ...
        try:
            ...
            self._cleanup()
            if not hasattr(self.functional, 'Extract'):
                return True
            if self.functional.Extract(self.outdir).success:
                return True
    ...
    def start(self, comm):
        """ Starts current job. """
        if super(CallProcess, self).start(comm):
            return True
    ...
    def wait(self):
        ...
        if super(CallProcess, self).wait():
            return True
