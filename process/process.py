...
class Process(object):
    ...
    def start(self, comm):
        ...
        if self.done:
            return True
        if self.started:
            raise AlreadyStarted('start cannot be called twice.')
        self.started = True
        if comm is not None:
            if comm['n'] == 0:
                raise MPISizeError('Empty communicator passed to process.')
    ...
    def _cleanup(self):
        ...
        """
        try:
            if hasattr(self.process, '_cleanup'):
                self.process._cleanup()
        finally:
            self.process = None
