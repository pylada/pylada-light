...
def test_ediff():
    ...
    assert abs(float(eval(repr(o), d).output_map(structure=structure)
                     ['ediff']) - a.ediff) < a.ediff * 1e-2
    assert abs(float(loads(dumps(o)).output_map(structure=structure)
                     ['ediff']) - a.ediff) < a.ediff * 1e-2
    a.ediff = -1
    ...
    assert abs(float(eval(repr(o), d).output_map(
        structure=structure)['ediff']) - 2e-4 * N) < 2e-4 * 1e-2
...
def test_ediffg():
    ...
    assert abs(float(eval(repr(o), d).output_map(
        structure=structure)['ediffg']) - 2e-4 * N) < 2e-4 * 1e-2
    assert abs(float(loads(dumps(o)).output_map(structure=structure)
                     ['ediffg']) - 2e-4 * N) < 2e-4 * 1e-2
    a.ediffg = 2e-4
    a.ediffg_per_atom = -2e-4
    assert a.ediffg is None
    assert abs(a.ediffg_per_atom + 2e-4) < 1e-8
    ...
    assert abs(float(eval(repr(o), d).output_map(
        structure=structure)['ediffg']) + 2e-4) < 2e-4 * 1e-2
    assert abs(float(loads(dumps(o)).output_map(
        structure=structure)['ediffg']) + 2e-4) < 2e-4 * 1e-2
