...
def parse_incar(path):
    ...
    if isinstance(path, str):
        if path.find('\n') == -1:
            ...
            if isdir(dummy):
                dummy = join(dummy, 'INCAR')
            with open(dummy) as file:
                return parse_incar(file)
        else:
    ...
    for line in path:
        ...
        if len(dummy) and len(lines) == 0:
            lines.append(dummy.pop(-1))
        while len(dummy):
            if lines[-1][-1] == '\\':
    ...
    for line in lines:
        ...
        if len(keyword) == 0:
            raise ValueError('Found empty keword in INCAR.')
        if keyword in result:
            raise ValueError('Found duplicate keyword {0} in INCAR'.format(keyword))
