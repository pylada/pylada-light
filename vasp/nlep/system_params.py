...
class SystemParams:
    ...
    def __init__(self, cation, anion, potcar_dir, outcar_data_dir, floating_vbm, nlep_params=None):
        ...
        self.bounds = None
        # this leaves it to global default in input_generic.py or run_input.py;
        # otherwise set it to sys-specific value
        self.scale_pressure = None
...
# will just make that happen:
    ...
    # for setting up offsets to reference everything from VBM
        ...
        # 9      22.0443      22.0585      -7.8777      -7.8942      -3.2856
        # 0.8612       0.0000
        ...
        if (nlep_params != None and cation in nlep_params):
            self.vasp.add_specie = "%s" % cation, "%s/%s%s" % (
                potcar_dir, cation, self.cation_potcar_ext), nlep_params[cation]
        else:
            self.vasp.add_specie = "%s" % cation, "%s/%s%s" % (
                potcar_dir, cation, self.cation_potcar_ext), nlep_element_defaults[cation]

        if (nlep_params != None and anion in nlep_params):
            self.vasp.add_specie = "%s" % anion, "%s/%s%s" % (
                potcar_dir, anion, self.anion_potcar_ext), nlep_params[anion]
        else:
            self.vasp.add_specie = "%s" % anion, "%s/%s%s" % (
                potcar_dir, anion, self.anion_potcar_ext), nlep_element_defaults[anion]
