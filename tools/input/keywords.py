...
class BaseKeyword(object):
    ...
    def __repr__(self):
        ...
        if 'raw' in self.__dict__:
            args.append("raw={0.raw!r}".format(self))
...
class ValueKeyword(BaseKeyword):
    ...
    def _addrepr_args(self):
        ...
        if iargs.args is None or 'value' not in iargs.args:
            ...
            if iargs.defaults is None or -(i + 1) >= len(iargs.defaults):
                doaddval = True
            else:
                default = iargs.defaults[i]
                if repr(default) != repr(self.value):
                    doaddval = True
        if doaddval:
            args.append('value={0.value!r}'.format(self))
        return args
    ...
    def __repr__(self):
        ...
        if 'type' in self.__dict__:
            if isinstance(self.type, list):
                ...
                args.append('type={0.type.__name__}'.format(self))
        return '{0.__class__.__name__}({1})'.format(self, ', '.join(args))
...
class ChoiceKeyword(BaseKeyword):
    ...
    def _addrepr_args(self):
        ...
        if iargs.args is None or 'value' not in iargs.args:
            ...
            i = iargs.args.index('value') - len(iargs.args)
            if iargs.defaults is None or 1 - i < len(iargs.defaults):
                doaddval = True
            else:
                default = iargs.defaults[i]
                if repr(default) != repr(self.value):
    ...
    def __repr__(self):
        ...
        if 'values' in self.__dict__:
            args.append("values={0.values!r}".format(self))
        return "{0.__class__.__name__}({1})".format(self, ', '.join(args))
