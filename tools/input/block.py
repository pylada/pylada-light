...
class AttrBlock(BaseKeyword):
    ...
    def __setattr__(self, name, value):
        ...
        if isinstance(value, BaseKeyword):
            ...
            if not hasattr(value, 'keyword'):
                self._input[name].keyword = name
        elif name in self._input:
            result = self._input[name]
            if hasattr(result, '__set__'):
                result.__set__(self, value)
            else:
                self._input[name] = value
        else:
            super(AttrBlock, self).__setattr__(name, value)

    def __delattr__(self, name):
        """ passes through the input keywords in :py:attr:`_input`.  """
        if name in self._input:
            del self._input[name]
        else:
            super(AttrBlock, self).__delattr__(name)
    ...
    def add_keyword(self, name, value=None):
        ...
        # if a string, tries to guess what it is.
            ...
            if len(lines) == 1 and len(lines[0]) > 0:
                ...
                if all(isinstance(u, str) for u in n):
                    n = [lines]
    ...
    def __ui_repr__(self, imports, name=None, defaults=None, exclude=None):
        ...
        for key, value in self._input.items():
            ...
            if hasattr(value, '__ui_repr__'):
                ...
                if defaults is not None and key in defaults._input                     \
                   and defaults._input[key] is None:
                       continue
            ...
            try:
                ...
                if key in self._input:
                    ...
                    if hasattr(newobject, 'read_input'):
                        ...
                        raise internal('Cannot read INCAR for {0}'.format(key))
                else:
