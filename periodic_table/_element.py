...
class Element(object):
    ...
    def electronic_configuration(self):
        ...
        if N == 29:
            ...
            return result
        result.append({'s': min(N - 18, 2), 'p': min(N - 30, 6) if N >
                       30 else 0, 'd': min(N - 20, 10) if N > 20 else 0})
        if N < 37:
            return result
        if N == 39:
            result.append({'s': 2, 'p': 0, 'd': 1})
        ...
        if N == 47:
            result.append({'s': 1, 'p': 0, 'd': 10})
            return result
        result.append({'s': min(N - 36, 2), 'p': min(N - 48, 6) if N >
                       48 else 0, 'd': min(N - 38, 10) if N > 38 else 0})
        if N < 55:
            return result
        if N == 57:
    ...
    def group(self):
        ...
        p = last.get('p', 0)
        d = last.get('d', 0)
        if self.atomic_number > 57 and self.atomic_number < 72:
            return "Lanthanide"
        elif self.atomic_number > 89 and self.atomic_number < 104:
            return "Actinide"
        elif s == 2 and p == 6:
            return "VIII"
        elif s + p + d == 1:
